...
const assetBaseUrl = normalizeBaseUrl(
  process.env.ASSET_BASE_URL ||
    ...
    'https://your-bucket.oss-cn-shanghai.aliyuncs.com/taro'
);
const outputPath = resolve(projectRoot, process.env.COS_MANIFEST_OUTPUT || 'public/cos-assets.json');
const cardExtension = normalizeExtension(process.env.ASSET_CARD_EXTENSION || '');

const decks = [
  {
    key: 'eastern',
    cosDir: process.env.ASSET_EASTERN_CARD_DIR || 'cards/eastern-mystic-tarot',
    cardBackDir: process.env.ASSET_EASTERN_BACK_DIR || process.env.ASSET_EASTERN_CARD_DIR || 'cards/eastern-mystic-tarot',
    localCandidates: [
  ...
  {
    key: 'chinese-ink',
    cosDir: process.env.ASSET_CHINESE_INK_CARD_DIR || 'cards/chinese-ink-tarot',
    cardBackDir: process.env.ASSET_CHINESE_INK_BACK_DIR || process.env.ASSET_CHINESE_INK_CARD_DIR || 'cards/chinese-ink-tarot',
    localCandidates: [
